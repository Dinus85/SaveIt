...
mixin FolderServiceCRUD on FolderServiceBase {
  ...
  String? validateFolderName(String name, {MockFolder? parent, MockFolder? excludeFolder}) {
    ...
    if (parent == null) {
      ...
      final exists = folders.any((f) => 
        ...
        (excludeFolder == null || f.name != excludeFolder.name || f.level != excludeFolder.level)
      );
      ...
      final exists = parent.children.any((child) =>
        ...
        // 🔥 FIX: Escludi la cartella corrente (per permettere cambio maiuscole/minuscole)
        (excludeFolder == null || child.name != excludeFolder.name || child.level != excludeFolder.level)
      );
  ...
  Future<String> createFolderHierarchyFromPath(
    ...
    print('DEBUG: Max retry: $maxRetries, Delay base: ${retryDelay.inMilliseconds}ms');
    ...
    try {
      ...
      print('DEBUG: Path parts: ${pathParts.join(" → ")} (${pathParts.length} livelli)');

      final depthValidation =
          _accessService.validateHierarchyDepthForCreation(pathParts.length);
      ...
      for (int i = 0; i < pathParts.length; i++) {
        ...
        print('DEBUG: --- Livello $i: "$folderName" (${isRoot ? "ROOT" : "CHILD"}) ---');
        
        // A. Invalida cache PRIMA di ogni lettura
        print('DEBUG: Invalidando cache folders...');
        DataService.instance.invalidateCache(folders: true, posts: true); // ⭐ Anche posts per mantenere sincronizzazione
        ...
        if (isRoot) {
          ...
          try {
            ...
            print('DEBUG: ✅ Root folder "$folderName" già esistente (ID: ${existingFolder.id})');
          } catch (e) {
          ...
          try {
            ...
            print('DEBUG: ✅ Child folder "$folderName" già esistente (ID: ${existingFolder.id}, parentId: ${existingFolder.parentId})');
          } catch (e) {
            print('DEBUG: ℹ️ Child folder "$folderName" non esiste sotto parent $currentParentId');
          }
        }
        ...
        if (existingFolder != null) {
          ...
          print('DEBUG: ➡️ Cartella già presente, passando al livello successivo');
          continue;
        }
        ...
        if (isRoot) {
          final rootCount =
              realFolders.where((folder) => !folder.isDefault && folder.parentId == null).length;
          final rootValidation =
              _accessService.validateRootFolderCreationWithCount(rootCount);
          if (rootValidation != null) {
            throw Exception(rootValidation);
          }
        } else {
          final childCount =
              realFolders.where((folder) => folder.parentId == currentParentId).length;
          if (i > AppAccessService.freeMaxFolderLevel) {
            final depthValidation =
                _accessService.validateHierarchyDepthForCreation(i + 1);
            if (depthValidation != null) {
              throw Exception(depthValidation);
            }
          }
          if (_accessService.isFree &&
              childCount >= AppAccessService.freeChildFoldersPerParentLimit) {
            throw Exception(
              'Versione Free: massimo ${AppAccessService.freeChildFoldersPerParentLimit} sottocartelle per cartella.',
            );
          }
        }
        ...
        for (int retry = 0; retry < maxRetries; retry++) {
          ...
          print('DEBUG: ⏳ Retry $retry/${maxRetries - 1}: attendendo ${delayMs}ms...');
          await Future.delayed(Duration(milliseconds: delayMs));
          
          // Invalida e ricarica completamente
          DataService.instance.invalidateCache(folders: true, posts: true); // ⭐ Anche posts
          await DataService.instance.reloadFromDisk();
          ...
          try {
            ...
            if (retry == maxRetries - 1) {
              ...
              print('DEBUG: ❌ Cartella "$folderName" non confermata dopo $maxRetries retry');
              ...
              try {
                ...
                print('DEBUG: ⚠️ Trovata cartella con nome corretto ma potrebbe avere parentId sbagliato');
                print('DEBUG: Usando ID: ${createdFolder.id} (parentId: ${createdFolder.parentId})');
                currentFolderId = createdFolder.id;
                currentParentId = createdFolder.id;
                creationConfirmed = true;
                break;
  ...
  Future<void> _createChildFolderWithParentId(String parentId, String folderName) async {
    ...
    final folderData = {
      ...
      'color': '#${FolderManagement.getRandomColor().value.toRadixString(16).substring(2)}',
    ...
    print('DEBUG: Child folder creata con ID: ${docRef.id}, parentId: $parentId');
  }
  ...
  Future<void> createPersistentFolder(String name) async {
    ...
    try {
      ...
      final rootValidation = _accessService.validateRootFolderCreation(folders);
      if (rootValidation != null) {
        throw Exception(rootValidation);
      }
  ...
  Future<void> createSubfolderInFolder(MockFolder parentFolder, String name) async {
    final subfolderValidation = _accessService.validateSubfolderCreation(parentFolder);
    ...
    try {
      ...
      final existingChild = parentFolder.children.firstWhere(
        ...
        orElse: () => MockFolder(name: '', count: '', color: Colors.grey, level: -1),
      );
      
      if (existingChild.level != -1) {
        print('DEBUG: ⚠️ Sottocartella "$normalizedName" esiste già localmente');
      ...
      if (parentId == null) {
        print('DEBUG: ID parent non trovato nel MockFolder, cerco nel database...');
        try {
          // Usa cache se disponibile, altrimenti rete (ma non forza refresh)
          final realFolders = await DataService.instance.getFolders();
          final realParent = findRealFolderByMockFolder(realFolders, parentFolder);
      ...
      if (parentId == null) {
        throw Exception('Cartella parent non trovata nel database (ID mancante)');
      }
      ...
      final newMockFolder = MockFolder(
        ...
        name: normalizedName,
        count: 'Vuota',
        color: FolderManagement.getRandomColor(), // Colore temporaneo, sarà sovrascritto dal DB
      ...
      final docRef = await foldersCollection.add(folderData);
      print('DEBUG: Sottocartella creata con ID: ${docRef.id}, parentId: $parentId');
  ...
  Future<void> deleteFolder(MockFolder folder) async {
    ...
    try {
      await executeAuthenticatedOperation(() async {
        ...
        if (folderId == null) {
          print('DEBUG: ID folder mancante nel MockFolder, cerco nella cache...');
          final realFolders = await DataService.instance.getFolders(forceRefresh: false);
          final realFolder = findRealFolderByMockFolder(realFolders, folder);
          folderId = realFolder?.id;
        }

        if (folderId == null) {
          print('ERRORE CRITICO: Cartella "${folder.name}" non trovata (ID mancante)');
          throw Exception(
            'Cartella "${folder.name}" non trovata. '
  ...
  Future<Map<String, dynamic>> analyzeDeletionImpact(MockFolder folder) async {
    ...
    postsAffected += allPosts.where((post) => post.sourceFolder == folder).length;
    ...
    void countRecursively(MockFolder currentFolder) {
      ...
      for (final child in currentFolder.children) {
        postsAffected += allPosts.where((post) => post.sourceFolder == child).length;
  ...
  Future<void> renameFolder(MockFolder folder, String newName) async {
    ...
    final validationError = validateFolderName(capitalizedName, parent: folder.parent, excludeFolder: folder);
    ...
    try {
      ...
      print('DEBUG: Cartella reale trovata: ${realFolder.name} (${realFolder.id})');
      ...
      print('DEBUG: Nome aggiornato in memoria: ${oldName} → ${capitalizedName}');
  ...
  Future<void> moveFolder(MockFolder folderToMove, MockFolder? destination) async {
    ...
    try {
      print('DEBUG: Spostamento cartella "${folderToUse.name}" verso "${destinationToUse?.name ?? "Home"}"');
      ...
      if (!removed) {
        print('WARNING: Impossibile trovare la cartella da rimuovere nella struttura locale');
      }
      ...
      if (destinationToUse != null) {
        ...
        destinationToUse.children = List.from(destinationToUse.children)..add(folderToUse);
      } else {
        folderToUse.level = 0;
      ...
      await executeAuthenticatedOperation(() async {
        ...
        Folder? realFolderToMove = findRealFolderByMockFolder(realFolders, folderToUse);
        
        // Fallback: Se non trovata per path esatto, cerca per nome (case-insensitive)
        if (realFolderToMove == null) {
          print('WARNING: Cartella sorgente non trovata per path, tento ricerca per nome...');
          try {
            realFolderToMove = realFolders.firstWhere(
              (f) => f.name.toLowerCase() == folderToUse.name.toLowerCase() && !f.isDefault
            );
            print('DEBUG: Fallback riuscito: trovata cartella ${realFolderToMove.name} (${realFolderToMove.id})');
          } catch (_) {
        ...
        if (realFolderToMove == null) {
          ...
          print('WARNING: Fallback fallito. Provo reload forzato cache e riprovo...');
          DataService.instance.invalidateCache(folders: true, posts: true);
          final freshFolders = await DataService.instance.getFolders();
          
          try {
            realFolderToMove = freshFolders.firstWhere(
              (f) => f.name.toLowerCase() == folderToUse.name.toLowerCase() && !f.isDefault
            );
          } catch (_) {}
          
          if (realFolderToMove == null) {
            throw Exception('Cartella sorgente "${folderToUse.name}" non trovata nel database');
          }
        }
        
        String? newParentId;
        if (destinationToUse != null) {
          Folder? realDestination = findRealFolderByMockFolder(realFolders, destinationToUse);
          
          // Fallback anche per destinazione
          if (realDestination == null) {
             print('WARNING: Cartella destinazione non trovata per path, tento ricerca per nome...');
             try {
                realDestination = realFolders.firstWhere(
                  (f) => f.name.toLowerCase() == destinationToUse!.name.toLowerCase() && !f.isDefault
                );
             } catch (_) {}
          }
          
          if (realDestination == null) {
            throw Exception('Cartella destinazione "${destinationToUse.name}" non trovata nel database');
          }
          newParentId = realDestination.id;
        }
        
        print('DEBUG: Aggiornamento DB - FolderID: ${realFolderToMove.id}, NewParentID: $newParentId');
  ...
  Future<void> updatePostTags(String postId, List<String> newTags) async {
    try {
      await executeAuthenticatedOperation(() async {
        ...
        print('DEBUG: Hashtag aggiornati per post $postId: ${newTags.join(", ")}');
      }, 'update_post_tags');
  ...
  List<MockPost> getAllPostsInFolderRecursive(MockFolder folder) {
    ...
    void collectPostsRecursively(MockFolder currentFolder) {
      allPostsInHierarchy.addAll(
        allPosts.where((post) => post.sourceFolder == currentFolder)
      );
